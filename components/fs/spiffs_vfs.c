...
#define CONFIG_SPIFFS_BASE_ADDR 	0x3d0000
#define CONFIG_SPIFFS_SIZE 		0x20000
#define CONFIG_SPIFFS_LOG_BLOCK_SIZE 	8192*2
...
static int IRAM_ATTR vfs_spiffs_open(const char *path, int flags, int mode) {
    ...
    int res = fs_list_add(&files, file, &fd);
    if (res) {
    	free(file);
    	errno = res;
    	return -1;
...
static ssize_t IRAM_ATTR vfs_spiffs_write(int fd, const void *data, size_t size) {
    ...
    res = fs_list_get(&files, fd, (void **)&file);
    if (res) {
		errno = EBADF;
		return -1;
    }
